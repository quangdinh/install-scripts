...
xfce_multimedia = True
q = request_input("Do you want to install Xfce Multimedia? [Yes]/No ")
if q.lower() == "no" or q.lower() == "n":
...
if disk != "None":
  ...
  cmdLine = '"' + cmdLine + ' quiet loglevel=3 splash rd.systemd.show_status=auto rd.udev.log_priority=3 module_blacklist=iTCO_wdt,iTCO_vendor_support'+ cmdLineExtra + '"'
...
print_task("Installing Openssh")
run_chroot("/usr/bin/pacman", "-S --noconfirm", "openssh")
print("Done")
...
if xfce:
  ...
  print_task("Installing lightdm")
  run_chroot("/usr/bin/pacman", "-S --noconfirm", "lightdm lightdm-gtk-greeter")
